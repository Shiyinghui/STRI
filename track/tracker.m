function tracker(varargin)
    ...
    % load video info
    [img_files, target_pos, target_size] = load_video_info(p.seq_base_path, p.dataset, p.video);
    ...
    imwrite(gather(z_crop)/255, 'bolt_crop.jpg');
    ...
    for i = 1:nImgs
        ...
        if p.visualization
            im = gather(im)/255;
            if p.siamfc
                im = insertShape(im, 'rectangle', net1_rect_pos, 'LineWidth', 2, 'Color', 'red');
            end
            if p.siamfc_tri
