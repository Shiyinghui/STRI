classdef XCorr < dagnn.Layer
    ...
    methods
        function outputs = forward(obj, inputs, params)
            ...
            z = inputs{1}; % z denotes the exemplar
            x = inputs{2}; % x denotes the instance
            assert(ndims(z) == ndims(x), 'z and x have different numer of dimensions');
            assert(size(z,1) <= size(x,1), 'z has to be smaller than x');
            [wx,hx,cx,bx] = size(x);
